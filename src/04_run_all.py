...
def main() -> None:
    ...
    print(analytical_df[["target", "model", "g_bar(x)", "bias", "variance", "expected_out_error"]])

    print("\n=== Simulation ===")
    simulation_df = simulation.run_simulation(n_datasets=300_000, seed=42)
    print(simulation_df)

    print("\n=== Learning curves ===")
    lc_df = learning_curve.run_learning_curves(noise_values=[0.0, 0.30])
    print(lc_df.head(12))

    print("\n=== Bias-Variance decomposition figure ===")
    fig_path, csv_path = visualization.plot_bias_variance_decomposition(noise_std=0.20, show=False)
